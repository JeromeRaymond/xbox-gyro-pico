...
class MCP():
    def __init__(self, name, csPin):    
        if (name == "MCP4922"):
            self.spiInstance = SoftSPI(baudrate=1000000, sck=Pin(14), mosi=Pin(15), miso=Pin(4))
            self.dataBitSize = 12
            self.gain = True
            self.vRef = 3.33
    ...
    def __genFrameA(self, value):
        tempVal = self.bufferMode*2**14+self.gain*2**13+self.isOn*2**12+(value<<(12-self.dataBitSize))
        return tempVal.to_bytes(2, 'big')

    def __genFrameB(self, value):
        tempVal = 1*2**15+self.bufferMode*2**14+self.gain*2**13+self.isOn*2**12+(value<<(12-self.dataBitSize))# 1*2**15 pour selection chan B
        return tempVal.to_bytes(2, 'big')
    ...
    def writevalueA(self, value):
        frameA = self.__genFrameA(value)
        self.chipSelectPin.low()
        self.spiInstance.write(frameA)
        self.chipSelectPin.high()
        
    def writevalueB(self, value):
        frameB = self.__genFrameB(value)
        self.chipSelectPin.low()
        self.spiInstance.write(frameB)
        self.chipSelectPin.high()
#print(frame) #Debug
